...
def logement_home(request):
    """Page d'accueil des logements - Liste tous les logements approuvés"""
    logements = Logement.objects.filter(available=True, approved=True, rejected=False).order_by('-created_at')
...
def logement_messages(request, logement_id):
    ...
    if request.user == logement.owner:
        messages.info(request, "Vous ne pouvez pas vous envoyer un message à vous-même.")
        return redirect('logement_detail', logement_id=logement_id)
    
    # Récupérer les messages marketplace entre l'utilisateur et le propriétaire du logement
    messages_list = Message.objects.filter(
        Q(sender=request.user, receiver=logement.owner) |
        Q(sender=logement.owner, receiver=request.user),
        deleted=False,
        logement=logement  # Seulement les messages de ce logement
    ).order_by('sent_at')
    
    # Marquer les messages comme lus
    Message.objects.filter(
        sender=logement.owner, 
...
def marketplace_messages(request):
    ...
    # Récupérer tous les logements avec lesquels l'utilisateur a échangé des messages
    logements_with_messages = Logement.objects.filter(
        messages__sender=request.user,
        messages__deleted=False
    ).distinct() | Logement.objects.filter(
        messages__receiver=request.user,
        messages__deleted=False
    ).distinct()
    
    conversations = []
    for logement in logements_with_messages.distinct():
