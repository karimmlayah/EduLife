...
function toggleForm(isRegister) {
    ...
    // Réinitialiser les widgets reCAPTCHA lors du basculement
    if (typeof grecaptcha !== 'undefined') {
        try {
            if (isRegister) {
                // Réinitialiser le widget de login
                const loginRecaptcha = document.getElementById('login-recaptcha');
                if (loginRecaptcha) {
                    grecaptcha.reset('login-recaptcha');
                }
            } else {
                // Réinitialiser le widget d'enregistrement
                const registerRecaptcha = document.getElementById('register-recaptcha');
                if (registerRecaptcha) {
                    grecaptcha.reset('register-recaptcha');
                }
            }
        } catch (e) {
...
async function registerFaceID() {
    ...
    // Vérifier le hostname pour déterminer la configuration WebAuthn
    const hostname = window.location.hostname;
    const origin = window.location.origin;
    
    // Vérifications pour détecter les domaines invalides pour WebAuthn
    const isLocalhost = hostname === 'localhost' || hostname === '127.0.0.1' || hostname === '0.0.0.0' || hostname === '[::1]';
    const isIPAddress = /^\d+\.\d+\.\d+\.\d+$/.test(hostname);
    const isPrivateIP = hostname.startsWith('192.168.') || hostname.startsWith('10.') || hostname.startsWith('172.') || hostname.startsWith('169.254.');
    const hasValidDomainFormat = hostname.includes('.') && hostname.split('.').length >= 2;
    
    // Un domaine valide doit avoir au moins un point et ne pas être localhost/IP
    const isValidDomain = hasValidDomainFormat && !isLocalhost && !isIPAddress && !isPrivateIP;
    
    // Pour localhost/IP, on omettra le champ 'id' dans 'rp' pour permettre WebAuthn
    const useLocalhostMode = isLocalhost || isIPAddress || isPrivateIP || !hasValidDomainFormat;
    ...
    try {
        ...
        // Pour localhost/IP, on omet complètement le champ 'id' pour permettre WebAuthn
        // WebAuthn utilisera automatiquement l'origine actuelle comme domaine
        const publicKeyCredentialCreationOptions = {
            challenge: challenge,
            rp: useLocalhostMode ? {
                // En localhost, on omet 'id' - WebAuthn utilisera l'origine par défaut
                name: "GL Project"
            } : {
                // En production avec domaine valide, on spécifie l'id
                name: "GL Project",
                id: hostname
            },
            user: {
        ...
        // Vérification finale du domaine avant l'appel WebAuthn
        console.log('Tentative d\'enregistrement Face ID avec domaine:', hostname, 'Mode localhost:', useLocalhostMode);
        console.log('Configuration rp:', publicKeyCredentialCreationOptions.rp);
        ...
        try {
            ...
            // Si l'erreur persiste avec localhost, suggérer HTTPS ou un tunnel
            if (webauthnError.message && (
                webauthnError.message.includes('invalid domain') || 
                webauthnError.message.includes('Invalid domain') ||
                webauthnError.message.includes('not a valid domain') ||
                webauthnError.message.includes('This is an invalid domain')
            )) {
                const errorMsg = useLocalhostMode 
                    ? '❌ Erreur: WebAuthn ne fonctionne pas avec localhost/IP.\n\n' +
                      '📋 Solutions pour tester Face ID en local:\n\n' +
                      '1️⃣ Utiliser un tunnel HTTPS (recommandé):\n' +
                      '   • Installer: npm install -g localtunnel\n' +
                      '   • Lancer: npx localtunnel --port 8000\n' +
                      '   • Utiliser l\'URL HTTPS fournie\n\n' +
                      '2️⃣ Configurer un domaine local:\n' +
                      '   • Ajouter dans C:\\Windows\\System32\\drivers\\etc\\hosts:\n' +
                      '     127.0.0.1  monapp.local\n' +
                      '   • Accéder via: http://monapp.local:8000\n\n' +
                      '3️⃣ Utiliser HTTPS localement (plus complexe)\n\n' +
                      '💡 En production, utilisez un domaine valide (ex: example.com)'
                    : 'Erreur: Domaine invalide pour WebAuthn.\n\n' +
                      'Veuillez utiliser un domaine valide (ex: example.com).';
                
                if (typeof showToast !== 'undefined') {
                    showToast(useLocalhostMode 
                        ? 'WebAuthn nécessite un domaine valide. Utilisez un tunnel HTTPS (localtunnel/ngrok).' 
                        : 'Domaine invalide pour WebAuthn.', 'error');
                } else {
                    alert(errorMsg);
                }
                console.error('Erreur WebAuthn - Domaine invalide:', {
                    hostname: hostname,
                    origin: origin,
                    useLocalhostMode: useLocalhostMode,
                    error: webauthnError
                });
                return;
            }
        ...
        if (error.name === 'NotAllowedError') {
            if (typeof showToast !== 'undefined') {
                showToast('L\'authentification biométrique a été annulée ou refusée.', 'warning');
            } else {
            ...
            error.message.includes('Invalid domain') ||
            error.message.includes('not a valid domain')
        )) {
            const errorMsg = 'Erreur: Domaine invalide pour WebAuthn.\n\n' +
                            'Si vous êtes en localhost, assurez-vous d\'utiliser HTTPS ou un tunnel (ngrok, localtunnel).\n\n' +
                            'En production, utilisez un domaine valide (ex: example.com).';
            
            if (typeof showToast !== 'undefined') {
                showToast('Erreur de domaine. Vérifiez votre configuration.', 'error');
            } else {
                alert(errorMsg);
            }
        } else {
            const errorMsg = 'Erreur lors de l\'enregistrement Face ID: ' + (error.message || error.toString());
            if (typeof showToast !== 'undefined') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Rediriger vers la page d'enregistrement Face ID
    if (faceIdRegisterBtn) {
        faceIdRegisterBtn.addEventListener('click', function() {
            window.location.href = '/face-id/register/';
        });
    }
    ...
    console.log('Face ID activé pour:', window.location.hostname);
});
